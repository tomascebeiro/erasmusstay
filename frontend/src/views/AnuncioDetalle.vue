      ...
      <article v-else-if="anuncio" class="grid lg:grid-cols-2 gap-8">
        <section class="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <div v-if="anuncio.imagenes?.length" class="grid grid-cols-4 gap-3 mt-4">
            ...
            >
              <img :src="img.url" :alt="anuncio.titulo" class="w-full h-full object-cover">
            </button>
          </div>
        </section>
        ...
        <section class="bg-white rounded-xl border border-slate-200 p-6 md:p-8">
          <div class="flex flex-wrap gap-2 mb-4">
            <span v-if="anuncio.aprobado" class="bg-green-100 text-green-700 text-xs font-bold px-3 py-1 rounded-full">
              Aprobado
            </span>
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mt-6">
            <div class="bg-slate-50 border border-slate-200 rounded-lg p-3">
              ...
              <span class="font-bold">{{ anuncio.duracion_min_meses }}-{{ anuncio.duracion_max_meses }} meses</span>
            </div>
        ...
        <section class="lg:col-span-2 bg-white rounded-xl border border-slate-200 p-6 md:p-8">
          ...
          <div v-if="anuncio.valoraciones?.length" class="space-y-4">
            ...
            >
              <div class="flex justify-between gap-4">
                ...
                <span class="font-bold text-yellow-600">{{ valoracion.puntuacion }}/5</span>
              </div>
              <p class="text-slate-600 mt-2">{{ valoracion.comentario }}</p>
            </article>
          </div>
          ...
          <form v-if="canReview" @submit.prevent="submitRating" class="mt-8 border-t border-slate-200 pt-6">
            <h3 class="text-xl font-black text-slate-900 mb-4">
              Dejar valoración
            </h3>

            <div v-if="ratingError" class="bg-red-50 border border-red-200 text-red-700 rounded p-3 text-sm mb-4">
              {{ ratingError }}
            </div>

            <div v-if="ratingSuccess" class="bg-green-50 border border-green-200 text-green-700 rounded p-3 text-sm mb-4">
              {{ ratingSuccess }}
            </div>

            <div class="grid md:grid-cols-[160px_1fr] gap-4">
              <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Puntuación</label>
                <select v-model="ratingForm.puntuacion" class="w-full border border-slate-300 rounded-lg px-3 py-2">
              ...
              <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Comentario</label>
                <textarea v-model="ratingForm.comentario" rows="3" class="w-full border border-slate-300 rounded-lg px-3 py-2"></textarea>
              </div>
            </div>

            <button :disabled="loadingRating" class="mt-4 bg-blue-700 text-white font-bold px-5 py-3 rounded-lg disabled:opacity-50">
